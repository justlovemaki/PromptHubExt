...
import { TagLabel, TagCategory, TagClassification, TagsConfig } from '@/types';
...
export function getCurrentLocale(): string {
  // 从浏览器扩展的 i18n API 获取语言环境
  // 如果不可用，则从 navigator.language 获取
  if (typeof chrome !== 'undefined' && chrome.i18n) {
    const locale = chrome.i18n.getMessage('@@ui_locale');
    if (locale) {
      // 将 chrome locale 格式转换为我们的格式
...
export function getTagsConfigPath(locale: string = getCurrentLocale()): string {
  // 在浏览器扩展中，我们需要从扩展的资源路径加载标签配置
  // 根据当前语言环境选择对应的配置文件
  return chrome.runtime.getURL(`./tags/tags-${locale}.json`);
}
...
export async function loadTagsConfig(locale: string = getCurrentLocale()): Promise<TagsConfig | null> {
  ...
  try {
    ...
    if (locale !== 'en') {
      ...
      try {
        const fallbackResponse = await fetch(chrome.runtime.getURL('./tags/tags-en.json'));
